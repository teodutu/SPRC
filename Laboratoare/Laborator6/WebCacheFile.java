...
public class WebCacheFile {
	 ...
	 */
	public Hashtable<String, WebCacheEntry> init() {
		Hashtable<String, WebCacheEntry> h = new Hashtable<>();
		...
		// daca am gasit un director
		// nu e de interes, il sarim
		// altfel incercam si citim continutul intrarii cache in memorie
		IntStream.range(0, fs.length).filter(i -> fs[i].isFile()).forEach(i -> {
			try {
